...
%C是K*P，K个簇中心，
%n*1的IDX
%sumD是簇内到centroid的距离之和，1*K
%D是每个点到每个centroid的距离，N*K
%GZQ: x, N*P，待聚类数据(row = N, column = p)
%k个簇
%varargin：输入参数 variable argument input
...
%   correspond to points, columns correspond to variables.  Note: when X is
%   数据X的行对应数据X的关键点，X的列对应数据X的变量
...
%   to every centroid in the N-by-K matrix D. %D是每个点到每个centroid的距离，N*K
%
...
%   $Revision: 1.1.10.4 $  $Date: 2011/10/15 02:06:24 $

if nargin < 2 %输入参数少于2个，报错
    error(message('stats:kmeans:TooFewInputs'));
end

[~,wasnan,X] = statremovenan(X); %Remove NaN values from inputs
hadNaNs = any(wasnan);
if hadNaNs
...
pnames = {   'distance'  'start' 'replicates' 'emptyaction' 'onlinephase' 'options' 'maxiter' 'display'}; %MaxIter迭代次数限制
dflts =  {'sqeuclidean' 'sample'          []         'error'         'on'        []        []        []};
[distance,start,reps,emptyact,online,options,maxit,display] ...
    = internal.stats.parseArgs(pnames, dflts, varargin{:});

distNames = {'sqeuclidean','cityblock','cosine','correlation','hamming'}; %距离
distance = internal.stats.getParamVal(distance,distNames,'''Distance''');
switch distance %distance参数
    case 'cosine' %余弦距离
...
if ~isempty(maxit)
    options = statset(options,'MaxIter',maxit); %MaxIter迭代次数限制
end
...
maxit = options.MaxIter; %MaxIter迭代次数限制
...
for rep = 1:reps
    ...
    try % catch empty cluster errors and move on to next rep
        ...
        if display > 1 % 'final' or 'iter'
            fprintf('%s\n',getString(message('stats:kmeans:IterationsSumOfDistances',iter,sprintf('%g',totsumD) )));
        end
...
% Return the best solution %返回最优解
idx = idxBest;
C = Cbest;
sumD = sumDBest;
if nargout > 3
    ...
    function converged = batchUpdate
        ...
        while true
            ...
            if ~isempty(empties)
                switch emptyact
                    case 'error'
                        error(message('stats:kmeans:EmptyCluster', iter, repsMsg( rep, reps )));
                    case 'drop'
                        % Remove the empty cluster from any further processing
                        D(:,empties) = NaN;
                        changed = changed(m(changed) > 0);
                        warning(message('stats:kmeans:EmptyCluster', iter, repsMsg( rep, reps )));
                    case 'singleton'
                        warning(message('stats:kmeans:EmptyCluster', iter, repsMsg( rep, reps )));
...
if reps == 1
    ...
    s = sprintf('%s',getString(message('stats:kmeans:Replicate',rep)));
end
end % function
